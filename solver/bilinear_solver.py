...
class Bilinear_Solver(Solver):
  ...
  def regular_solve(self):
    ...
    if self.vertex_rect[0][0] != self.vertex_rect[1][0] and self.vertex_rect[2][0] != self.vertex_rect[3][0]:
      ...
      sys.exit()
    # check y-dim regular
    if self.vertex_rect[0][1] != self.vertex_rect[2][1] and self.vertex_rect[1][1] != self.vertex_rect[3][1]:
      print 'Y coord is not the same. Not regular box!'
      sys.exit()

    alpha = (self.dst_point[0] - self.vertex_rect[0][0]) / (self.vertex_rect[2][0] - self.vertex_rect[0][0])
